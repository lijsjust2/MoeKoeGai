<template>
    <transition name="fade">
        <div v-if="showQueue" class="queue-popup">
            ...
            <ul ref="queueListRef" class="queue-list" @scroll="handleScroll">
                <li v-for="(item, index) in musicQueueStore.queue" :key="item.id"
                    class="queue-item" :class="{ 'playing': currentSong.hash == item.hash }">
...
const openQueue = async () => {
    ...
    if (showQueue.value) {
        ...
        setTimeout(() => {
            ...
            if (currentIndex !== -1 && queueListRef.value) {
                const targetElement = queueListRef.value.children[currentIndex];
                if (targetElement) {

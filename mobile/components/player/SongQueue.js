...
export default function useSongQueue(t, musicQueueStore, queueList = null) {
    ...
    const addSongToQueue = async (hash, name, img, author, isReset = true, formatRetryCount = 0) => {
        ...
        try {
            ...
            if (DEV) console.log('[SongQueue] 获取歌曲:', hash, name);
            ...
            // 根据用户设置确定请求参数
            const MoeAuth = typeof MoeAuthStore === 'function' ? MoeAuthStore() : { isAuthenticated: false, autoClaimVip: () => {} };
            const isAuth = !!MoeAuth.isAuthenticated;

            if (isAuth) {
                const now = Date.now()
                if (now - lastVipCheck >= VIP_CHECK_INTERVAL) {
                    lastVipCheck = now
            ...
            if (response.extName == 'mp4') {
                if (formatRetryCount >= 2) {
                    if (DEV) console.warn('[SongQueue] MP4重试已达上限，跳过');
                    currentSong.value.author = currentSong.value.name = t('huo-qu-yin-le-shi-bai');
                    return { error: true };
                }
                if (DEV) console.log('[SongQueue] 歌曲格式为MP4，尝试获取其他格式');
                return addSongToQueue(hash, name, img, author, false, formatRetryCount + 1);
            }

            // 设置URL
            if (response.url && response.url[0]) {
                currentSong.value.url = response.url[0];
                if (DEV) console.log('[SongQueue] 获取到音乐URL:', currentSong.value.url);
            } else {
                if (DEV) console.error('[SongQueue] 未获取到音乐URL');
                currentSong.value.author = currentSong.value.name = t('huo-qu-yin-le-shi-bai');
                return { error: true };
            }

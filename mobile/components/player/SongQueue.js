...
export default function useSongQueue(t, musicQueueStore, queueList = null) {
    ...
    const checkAndClaimVip = async (MoeAuth) => {
        const todayKey = new Date().toISOString().split('T')[0]
        const lastClaimDate = localStorage.getItem('lastVipClaimDate')
        
        if (lastClaimDate === todayKey) {
            console.log('[SongQueue VIP检查] 今天已经领取过VIP，跳过检查')
            return
        }
        
        try {
            console.log('[SongQueue VIP检查] 开始获取VIP信息...')
            ...
            if (VipInfoResponse.status === 1) {
                ...
                if (isVip) {
                    console.log('[SongQueue VIP检查] 用户已经是VIP，无需领取')
                    localStorage.setItem('lastVipClaimDate', todayKey)
                } else {
                    console.log('[SongQueue VIP检查] 用户不是VIP，开始领取1天VIP...')
                    const dayVipResponse = await get('/youth/day/vip', { receive_day: todayKey })
                    if (dayVipResponse.status === 1) {
                        console.log('[SongQueue VIP领取] 1天VIP领取成功')
                        localStorage.setItem('lastVipClaimDate', todayKey)
                        
                        await new Promise(resolve => setTimeout(resolve, 500))
                        
                        console.log('[SongQueue VIP升级] 开始升级为概念版VIP...')
                        const upgradeResponse = await get('/youth/day/vip/upgrade')
                        if (upgradeResponse.status === 1) {
                            console.log('[SongQueue VIP升级] 概念版VIP升级成功')
                        } else {
                            console.log('[SongQueue VIP升级] 概念版VIP升级失败:', upgradeResponse.data)
                        }
                        
                        console.log('[SongQueue VIP检查] 重新获取VIP信息...')
                        ...
                        if (newVipInfoResponse.status === 1) {
                            ...
                            await MoeAuth.setData({ Config: currentConfig })
                            console.log('[SongQueue VIP检查] VIP信息已更新')
                        }
                    } else {
                        console.log('[SongQueue VIP领取] 1天VIP领取失败:', dayVipResponse.data)
                        localStorage.setItem('lastVipClaimDate', todayKey)
                    }
                }
            } else {
                console.log('[SongQueue VIP检查] 获取VIP信息失败:', VipInfoResponse.data)
                localStorage.setItem('lastVipClaimDate', todayKey)
            }
        } catch (error) {
            console.error('[SongQueue VIP检查] VIP检查失败:', error)
            localStorage.setItem('lastVipClaimDate', todayKey)
        }
    }
    ...
    const addSongToQueue = async (hash, name, img, author, isReset = true) => {
        ...
        try {
            ...
            if (isAuth) {
                await checkAndClaimVip(MoeAuth)
            }
    ...
    const addLocalMusicToQueue = async (localItem, isReset = true) => {
        ...
        try {
            ...
            currentSong.value.img = localItem.cover || '';

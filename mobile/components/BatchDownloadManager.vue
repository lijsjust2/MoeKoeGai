...
const checkAndClaimVip = async () => {
    ...
    if (lastClaimDate === todayKey) {
        console.log('[BatchDownload VIP检查] 今天已经领取过VIP，跳过检查')
        return
    }
    
    try {
        console.log('[BatchDownload VIP检查] 开始获取VIP信息...')
        ...
        if (VipInfoResponse.status === 1) {
            ...
            if (isVip) {
                console.log('[BatchDownload VIP检查] 用户已经是VIP，无需领取')
                localStorage.setItem('lastVipClaimDate', todayKey)
            } else {
                console.log('[BatchDownload VIP检查] 用户不是VIP，开始领取1天VIP...')
                const dayVipResponse = await get('/youth/day/vip', { receive_day: todayKey })
                if (dayVipResponse.status === 1) {
                    console.log('[BatchDownload VIP领取] 1天VIP领取成功')
                    localStorage.setItem('lastVipClaimDate', todayKey)
                    
                    await new Promise(resolve => setTimeout(resolve, 500))
                    
                    console.log('[BatchDownload VIP升级] 开始升级为概念版VIP...')
                    const upgradeResponse = await get('/youth/day/vip/upgrade')
                    if (upgradeResponse.status === 1) {
                        console.log('[BatchDownload VIP升级] 概念版VIP升级成功')
                    } else {
                        console.log('[BatchDownload VIP升级] 概念版VIP升级失败:', upgradeResponse.data)
                    }
                    
                    console.log('[BatchDownload VIP检查] 重新获取VIP信息...')
                    ...
                    if (newVipInfoResponse.status === 1) {
                        ...
                        await MoeAuth.setData({ Config: currentConfig })
                        console.log('[BatchDownload VIP检查] VIP信息已更新')
                    }
                } else {
                    console.log('[BatchDownload VIP领取] 1天VIP领取失败:', dayVipResponse.data)
                    localStorage.setItem('lastVipClaimDate', todayKey)
                }
            }
        } else {
            console.log('[BatchDownload VIP检查] 获取VIP信息失败:', VipInfoResponse.data)
            localStorage.setItem('lastVipClaimDate', todayKey)
        }
    } catch (error) {
        console.error('[BatchDownload VIP检查] VIP检查失败:', error)
        localStorage.setItem('lastVipClaimDate', todayKey)
    }
}
    ...
    while (currentQualityIndex < qualityFallbackOrder.length) {
        ...
        try {
            ...
            if (response.status !== 1 || !response.url || !response.url[0]) {
                throw new Error(`获取 ${currentQuality}kbps 下载链接失败`);
            }
            ...
            lastError = error;
            console.warn(`歌曲 ${song.name} 尝试 ${currentQuality}kbps 失败:`, error.message);
            currentQualityIndex++;
        }
    }

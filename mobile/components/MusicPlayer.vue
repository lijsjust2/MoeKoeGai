...
const checkAndClaimVip = async () => {
  ...
  if (lastClaimDate === todayKey) {
    console.log('[MusicPlayer VIP检查] 今天已经领取过VIP，跳过检查')
    return
  }
  
  try {
    const currentUserId = MoeAuth.UserInfo?.userid || MoeAuth.UserInfo?.userId
    const cachedVipInfo = localStorage.getItem('cachedVipInfo')
    const cachedUserId = localStorage.getItem('cachedVipUserId')
    
    let vipInfoData = null
    ...
    if (cachedVipInfo && cachedUserId === currentUserId) {
      ...
      if (parsedVipInfo.is_vip === 1) {
        ...
        if (expireTime && new Date(expireTime) > new Date()) {
          isVipValid = true
          console.log('[MusicPlayer VIP检查] 缓存的基础VIP仍然有效，过期时间:', expireTime)
        }
      }
      ...
      if (!isVipValid && parsedVipInfo.busi_vip && Array.isArray(parsedVipInfo.busi_vip)) {
        ...
        if (hasValidBusiVip) {
          isVipValid = true
          console.log('[MusicPlayer VIP检查] 缓存的商业VIP仍然有效')
        }
      }
      
      if (isVipValid) {
        console.log('[MusicPlayer VIP检查] 使用缓存的VIP信息（VIP未过期且属于当前用户）...')
        vipInfoData = parsedVipInfo
      } else {
        console.log('[MusicPlayer VIP检查] 缓存的VIP已过期或用户不是VIP，开始获取VIP信息...')
        needRefreshVipInfo = true
      }
    } else {
      if (cachedVipInfo && cachedUserId !== currentUserId) {
        console.log('[MusicPlayer VIP检查] 缓存的VIP信息属于其他用户，开始获取VIP信息...')
      } else {
        console.log('[MusicPlayer VIP检查] 缓存不存在，开始获取VIP信息...')
      }
      needRefreshVipInfo = true
    }
    ...
    if (needRefreshVipInfo) {
      ...
      console.log('[MusicPlayer VIP检查] 获取VIP信息响应:', VipInfoResponse)
      ...
      if (VipInfoResponse.status === 1) {
        ...
        console.log('[MusicPlayer VIP检查] VIP信息已缓存，用户ID:', currentUserId)
        ...
        console.log('[MusicPlayer VIP检查] 获取VIP信息失败:', VipInfoResponse.data)
        return
      }
    }
    ...
    console.log('[MusicPlayer VIP检查] 最终VIP状态:', isVip)
    
    if (isVip) {
      console.log('[MusicPlayer VIP检查] 用户已经是VIP，无需领取')
      localStorage.setItem('lastVipClaimDate', todayKey)
    } else {
      console.log('[MusicPlayer VIP检查] 用户不是VIP，开始领取1天VIP...')
      const dayVipResponse = await get('/youth/day/vip', { receive_day: todayKey })
      console.log('[MusicPlayer VIP领取] 领取1天VIP响应:', dayVipResponse)
      
      if (dayVipResponse.status === 1) {
        console.log('[MusicPlayer VIP领取] 1天VIP领取成功')
        
        await new Promise(resolve => setTimeout(resolve, 500))
        
        console.log('[MusicPlayer VIP升级] 开始升级为概念版VIP...')
        const upgradeResponse = await get('/youth/day/vip/upgrade')
        console.log('[MusicPlayer VIP升级] 升级概念版VIP响应:', upgradeResponse)
        
        if (upgradeResponse.status === 1) {
          console.log('[MusicPlayer VIP升级] 概念版VIP升级成功')
        } else {
          console.log('[MusicPlayer VIP升级] 概念版VIP升级失败:', upgradeResponse.data)
        }
        
        // 领取VIP成功后，重新获取VIP信息并更新到存储中
        console.log('[MusicPlayer VIP检查] 重新获取VIP信息...')
        const updatedVipInfoResponse = await get('/youth/union/vip')
        console.log('[MusicPlayer VIP检查] 重新获取VIP信息响应:', updatedVipInfoResponse)
        ...
        if (updatedVipInfoResponse.status === 1) {
          ...
          console.log('[MusicPlayer VIP检查] VIP信息已更新到存储')
          
          // 更新缓存，同时存储用户ID
          localStorage.setItem('cachedVipInfo', JSON.stringify(updatedVipInfoResponse.data))
          localStorage.setItem('cachedVipUserId', currentUserId)
          console.log('[MusicPlayer VIP检查] VIP信息缓存已更新，用户ID:', currentUserId)
          ...
          if (afterClaimVipData.busi_vip && Array.isArray(afterClaimVipData.busi_vip)) {
            const hasValidVipAfterClaim = afterClaimVipData.busi_vip.some(vip => {
              if (!vip) return false
              const expireTime = vip.vip_end_time || vip.end_time
              return expireTime && new Date(expireTime) > new Date()
            })
            if (hasValidVipAfterClaim) {
              isVipAfterClaim = true
            }
          }
          
          if (isVipAfterClaim) {
            console.log('[MusicPlayer VIP检查] 领取后用户已成为VIP')
          } else {
            console.log('[MusicPlayer VIP检查] 领取后用户仍不是VIP')
          }
        } else {
          console.log('[MusicPlayer VIP检查] 重新获取VIP信息失败:', updatedVipInfoResponse.data)
        }
      } else {
        console.log('[MusicPlayer VIP领取] 1天VIP领取失败:', dayVipResponse.data)
      }
    ...
    console.error('[MusicPlayer VIP检查] 检查和领取VIP时出错:', error)
    localStorage.setItem('lastVipClaimDate', todayKey)
  }
}
...
const loadAudio = async (song) => {
  ...
  try {
    ...
    if (!MoeAuth.isAuthenticated || !checkIsVip()) {
      data.free_part = 1
    }
